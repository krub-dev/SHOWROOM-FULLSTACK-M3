...
describe("Showroom API - CRUD Integration Tests", () => {
	...
	beforeAll(async () => {
		...
		if (maxProject) {
			...
			console.log(
				`🔄 Reset ID sequence to start from ${maxProject.id + 1}`
			);
		}
	});
	...
	describe("GET /api/projects", () => {
		...
		it("should get a single project by ID", async () => {
			const response = await request(BASE_URL).get("/api/projects/1");
		...
		it("should return 404 for non-existent project ID", async () => {
			const response = await request(BASE_URL).get("/api/projects/99999");
	...
	describe("CRUD Lifecycle (CREATE → UPDATE → DELETE)", () => {
		...
		it("should delete the created project with valid authentication", async () => {
			...
			const deletedProject = getResponse.body.data.find(
	...
	describe("DELETE /api/projects/:id - Authentication & Validation", () => {
		it("should return 401 without authentication", async () => {
			const response = await request(BASE_URL).delete("/api/projects/1");
